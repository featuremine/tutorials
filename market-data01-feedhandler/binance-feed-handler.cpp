...
static void binance_feed_handler_component_process_one(struct fmc_component *self,
                                       struct fmc_reactor_ctx *ctx,
                                       fmc_time64_t now) noexcept {
  struct binance_feed_handler_component *comp = (binance_feed_handler_component *)self;
  try {
    if (comp->process_one())
        _reactor->queue(ctx);
...
static struct binance_feed_handler_component *binance_feed_handler_component_new(struct fmc_cfg_sect_item *cfg,
                                                 struct fmc_reactor_ctx *ctx,
...
struct fmc_cfg_node_spec *binance_feed_handler_cfgspecptr = binance_feed_handler_cfgspec;
...
FMCOMPMODINITFUNC void FMCompInit_binance_feed_handler(struct fmc_component_api *api,
